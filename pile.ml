...
match pile with
|[]-> failwith "Pile vide";
|(n,achanger)::t -> backtrack_changer n (ref achanger) tableau;pil:=t;backtrack_rajouter (-n) arajouter tableau;n;;

let rec update_supprimer n pil tableau liste=
match liste with
|[] -> ();
|h::t -> let (boole,liste)=tableau.(h) in tableau.(h) <- (boole,(List.filter (fun i -> i!=n) liste));if tableau.(h)=(boole,[]) then begin tableau.(0)<- false,[]; update_supprimer n pil tableau t end else update_supprimer n pil tableau t;;

let rec update_changer n pil tableau liste=
match liste with
|[]-> [];
|h::t -> let (boole,liste)=tableau.(h) in if not boole then tableau.(h) <- (true,liste);(h::(update_changer n pil tableau t));;
